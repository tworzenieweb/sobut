 ...
 *  @version  Release: $Revision: 16953 $
...
class ProductCore extends ObjectModel {
    ...
    public function addProductAttribute($price, $weight, $unit_impact, $ecotax, $quantity, $id_images, $reference, $supplier_reference, $ean13, $default, $location = NULL, $upc = NULL, $minimal_quantity = 1) {
        ...
        Db::getInstance()->AutoExecute(_DB_PREFIX_ . 'product_attribute', array('id_product' => (int) ($this->id), 'price' => (float) ($price), 'ecotax' => (float) ($ecotax), 'quantity' => 0,
            ...
            'location' => pSQL($location), 'ean13' => pSQL($ean13), 'upc' => pSQL($upc), 'default_on' => (int) ($default), 'minimal_quantity' => (int) $minimal_quantity), 'INSERT');
     ...
     */
    public static function getNewProducts($id_lang, $pageNumber = 0, $nbProducts = 10, $count = false, $orderBy = NULL, $orderWay = NULL, $getManufacturers = false) {
        if ($pageNumber < 0)
            $pageNumber = 0;
        if ($nbProducts < 1)
            $nbProducts = 10;
        ...
        if ($getManufacturers) {
            ...
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
			SELECT DISTINCT m.id_manufacturer, m.name AS manufacturers
			FROM `' . _DB_PREFIX_ . 'product` p
        ...
        if ($orderBy == 'manufacturer_name' && is_numeric($orderWay)) {
            ...
            $sql = '
                    ...
                    AND m.id_manufacturer = ' . $orderWay
                    . ' ORDER BY ' . (isset($orderByPrefix) ? pSQL($orderByPrefix) . '.' : '') . '`' . pSQL($orderBy) . '`
    ...
    public function getManufacturersForProduct(array $products) {
        $manufacturerIds = array();

        foreach ($products as $product) {

            $manufacturerIds[] = $product['id_manufacturer'];
        }



        $result = $manufacturerIds ? Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS(sprintf('
			SELECT DISTINCT m.id_manufacturer, m.name AS manufacturers
			FROM `' . _DB_PREFIX_ . 'manufacturer` m
			WHERE m.`id_manufacturer` in (%s) ORDER BY m.name', implode(',', $manufacturerIds))) :
                0;
